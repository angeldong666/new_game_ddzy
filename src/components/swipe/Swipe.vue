...
export default{
  ...
  props: {
    ...
    orientation: {
      type: String,
      default: 'horizontal' // 可选值有：horizontal | vertica
    }
  },
  filters: {
    getUrl: function (id) {
  ...
  methods: {
    ...
    doAnimate (towards, options) {
      ...
      var prevPage, nextPage, currentPage, moveHSpace, offsetTop
      ...
      if (!options) {
        // 滚动方向
        moveHSpace = this.orientation === 'horizontal' ? this.$el.clientWidth : this.$el.clientHeight
        ...
        if (prevPage) {
          ...
          this.translate(prevPage, -moveHSpace)
        }
        if (nextPage) {
          nextPage.style.display = 'block'
          this.translate(nextPage, moveHSpace)
        }
      } else {
        prevPage = options.prevPage
        currentPage = options.currentPage
        nextPage = options.nextPage
        moveHSpace = options.moveHSpace
      ...
      setTimeout(() => {
        if (towards === 'next') {
          this.translate(currentPage, -moveHSpace, speed, callback)
          if (nextPage) {
          ...
          if (typeof offsetTop !== 'undefined') {
            if (prevPage && offsetTop > 0) {
              this.translate(prevPage, moveHSpace * -1, speed)
            }
            if (nextPage && offsetTop < 0) {
              this.translate(nextPage, moveHSpace, speed)
            }
          } else {
            if (prevPage) {
              this.translate(prevPage, moveHSpace * -1, speed)
    ...
    next () {
      this.doAnimate('next')
      // this.translate(document.getElementById('test'),-30,300)
    },
    prev () {
      this.doAnimate('prev')
    },
